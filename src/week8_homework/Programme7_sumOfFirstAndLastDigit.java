 ...
 */
public class Programme7_sumOfFirstAndLastDigit
{
    //main method
    ...
    public static int sumFirstAndLastDigit(int number) //static method declare
    {
        int first = number;
        int last = 0;
        if(number>=0) //logic to find sum of first digit and last digit
        {
            int sum = 0;
            last = number%10; //last digit print
            while (first>=10)
            {
                first = first/10; //first digit display
            }
            sum = first+last;
            return sum;
        }
